...
SOURCE_URLS_FILE = os.path.join(BASEDIR, "config", "source_urls.txt")
RUNTIME_DIR = os.path.join(BASEDIR, "runtime")                  # kernels & db
SINGBOX_BIN = os.path.join(RUNTIME_DIR, "sing-box")


def load_source_urls(path=None) -> list:
    """从文本配置读取并校验上游订阅地址。"""
    config_path = path or SOURCE_URLS_FILE
    try:
        with open(config_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
    except FileNotFoundError as exc:
        raise FileNotFoundError(f"订阅源配置文件不存在: {config_path}") from exc
    ...
    for line_number, raw_line in enumerate(lines, 1):
        ...
        if any(char.isspace() for char in url):
            print(f"[!] 订阅源配置第 {line_number} 行含未编码空白，已跳过: {url!r}")
            continue
        try:
        ...
        if (parsed is None or parsed.scheme.lower() not in {"http", "https"}
            ...
            print(f"[!] 订阅源配置第 {line_number} 行 URL 无效，已跳过: {url!r}")
            continue
        if url in seen:
            print(f"[!] 订阅源配置第 {line_number} 行 URL 重复，已跳过: {url!r}")
            continue
        seen.add(url)
        urls.append(url)

    if not urls:
        raise ValueError(f"订阅源配置文件没有有效 URL: {config_path}")
    return urls
...
# proxy-checker 来源只返回 HTTP/SOCKS 代理，不进入旧的节点 URI 解析器。
# 这里仅登记已知来源的协议提示；普通订阅 URL 仍完全沿用原有路径。
_PROXY_SOURCE_MARKERS = (
    "proxifly/free-proxy-list", "databay-labs/free-proxy-list",
    "iplocate/free-proxy-list", "roosterkid/openproxylist",
    "thespeedx/socks-list", "vpslabcloud/vpslab-free-proxy-list",
    "hookzof/socks5_list", "spys.me", "api.proxyscrape.com",
    "proxylist.geonode.com", "www.my-proxy.com", "www.proxynova.com",
    "hide.mn", "free-proxy-list.net", "api.checkerproxy.net",
)
_PROXY_SCHEME_RE = re.compile(r"(?i)\b(https?|socks4a?|socks5h?)://([^\s\"'<>]+)")
_PROXY_IP_PORT_RE = re.compile(r"(?<![\w.])((?:\d{1,3}\.){3}\d{1,3}):(\d{1,5})(?!\d)")


def proxy_source_hint(url: str):
    """返回代理源的固定协议提示；混合源返回 None，不猜测协议。"""
    low = url.lower().rstrip("/")
    if any(marker in low for marker in _PROXY_SOURCE_MARKERS):
        for marker, proto in (
            ("socks5", "socks5"), ("socks4", "socks4"),
            ("proxytype=socks5", "socks5"), ("proxytype=socks4", "socks4"),
            ("proxytype=http", "http"), ("/http", "http"),
            ("/https", "http"), ("spys.me/proxy", "http"),
            ("spys.me/socks", "socks5"),
            ("my-proxy.com", "http"), ("proxynova.com", "http"),
            ("hide.mn", None), ("free-proxy-list.net", "socks5"),
        ):
            if marker in low:
                return proto
        return None
...
def _valid_proxy_host_port(host: str, port: int) -> bool:
...
def parse_proxy_endpoint(value, protocol_hint=None):
    """将显式代理地址转换为 (raw, outbound, server, port, proto)。"""
    value = str(value or "").strip().rstrip(",;\\\"'\\r")
    if not value:
        return None
    scheme = ""
    hostinfo = value
    ...
    proto = {"https": "http", "socks4a": "socks4", "socks5h": "socks5"}.get(scheme, scheme)
    proto = proto or (str(protocol_hint or "").lower() if protocol_hint else "")
    if proto not in {"http", "socks4", "socks5"} or not _valid_proxy_host_port(host, port):
        return None
    outbound = {"type": "http" if proto == "http" else "socks",
                "tag": "node", "server": host, "server_port": int(port)}
    if proto != "http":
        outbound["version"] = proto[-1]
    if username:
    ...
    if username:
        ...
        auth += "@"
    raw = f"{proto}://{auth}{authority}:{int(port)}"
    return raw, outbound, host, int(port), proto


def _proxy_records_from_json(data):
    if isinstance(data, list):
        return data
    if isinstance(data, dict):
        for key in ("data", "proxies", "proxyList", "items"):
            if isinstance(data.get(key), list):
                return data[key]
    return []


def parse_proxy_text(text: str, protocol_hint=None) -> list:
    results, seen = [], set()
    if not text:
        return results
    values = [m.group(0) for m in _PROXY_SCHEME_RE.finditer(text)]
    values += [f"{ip}:{port}" for ip, port in _PROXY_IP_PORT_RE.findall(text)]
    for value in values:
        parsed = parse_proxy_endpoint(value, protocol_hint)
...
def parse_proxy_json(data, protocol_hint=None) -> list:
    results, seen = [], set()
    for item in _proxy_records_from_json(data):
        if isinstance(item, str):
            values = [item]
        elif isinstance(item, dict):
            values = [item.get("proxy") or item.get("url") or ""]
            if not values[0] and item.get("ip") and item.get("port"):
                proto = item.get("protocol") or item.get("type") or item.get("scheme")
                if not proto and isinstance(item.get("protocols"), list) and item["protocols"]:
                    proto = item["protocols"][0]
                proto = proto or protocol_hint
                values = [f"{proto + '://' if proto else ''}{item['ip']}:{item['port']}"]
        else:
            continue
        for value in values:
            parsed = parse_proxy_endpoint(value, protocol_hint)
...
def is_proxy_source(url: str) -> bool:
    return proxy_source_hint(url) is not False


def fetch_proxy_candidates(source_urls) -> list:
    """抓取明确登记的代理池源；返回与旧候选一致的五元组。"""
    candidates, seen = [], set()

    def fetch_one(url):
        hint = proxy_source_hint(url)
        try:
            response = http_get(url, timeout=30)
            if response.status_code != 200:
                return url, [], f"HTTP {response.status_code}"
            if "proxifly/free-proxy-list" in url.lower():
                parsed = parse_proxy_json(response.json(), hint)
            elif "geonode.com/api" in url.lower():
                parsed = parse_proxy_json(response.json(), hint)
            elif "checkerproxy.net" in url.lower():
                archive = response.json()
                items = ((archive.get("data") or {}).get("items") or [])[:3]
                values = []
                for item in items:
                    date = item.get("date") if isinstance(item, dict) else ""
                    if not date:
                        continue
                    detail_url = f"https://api.checkerproxy.net/v1/landing/archive/{date}"
                    detail = http_get(detail_url, timeout=30)
                    if detail.status_code != 200:
                        continue
                    payload = detail.json()
                    values.extend(((payload.get("data") or {}).get("proxyList") or []))
                parsed = parse_proxy_text("\n".join(map(str, values)), "http")
            else:
                parsed = parse_proxy_text(response.text, hint)
            return url, parsed, None
        except Exception as exc:
            return url, [], str(exc)[:100]

    with ThreadPoolExecutor(MAX_WORKERS_FETCH) as ex:
        futures = [ex.submit(fetch_one, url) for url in source_urls]
        for future in as_completed(futures):
            url, found, error = future.result()
            if error:
                print(f"[!] 代理源拉取失败 {url} → {error}")
                continue
            print(f"[+] 代理源 {url} → {len(found)} 条")
            for item in found:
                if item[0] not in seen:
                    seen.add(item[0])
                    candidates.append(item)
    print(f"[*] 代理源解析总量: {len(candidates)}")
    return candidates


def fetch_raw_nodes(source_urls=None, include_proxy=False) -> list:
    nodes = set()
    source_urls = load_source_urls() if source_urls is None else source_urls
    if include_proxy:
        uri_source_urls = [u for u in source_urls if not is_proxy_source(u)]
        proxy_source_urls = [u for u in source_urls if is_proxy_source(u)]
        proxy_candidates = fetch_proxy_candidates(proxy_source_urls) if proxy_source_urls else []
    else:
        # 兼容旧调用方：默认模式完全沿用原始 URI 抓取，不请求代理池源。
        uri_source_urls = source_urls
        proxy_candidates = []
    print(f"[*] 抓取节点订阅源 ({len(uri_source_urls)} 个) ...")
    ...
    with ThreadPoolExecutor(MAX_WORKERS_FETCH) as ex:
        futs = [ex.submit(_fetch, u) for u in uri_source_urls]
        for f in as_completed(futs):
            ...
            nodes.update(got)
    print(f"[*] 初始抓取总量: {len(nodes)} 节点 URI + {len(proxy_candidates)} 代理")
    return (list(nodes), proxy_candidates) if include_proxy else list(nodes)
...
def outbound_to_clash(node: dict, name: str) -> dict:
    ...
    if t == "vless":
        ...
        proxy["type"] = "socks4" if str(node.get("version", "5")) == "4" else "socks5"
        proxy["udp"] = False
        if node.get("username"):
            proxy["username"] = node["username"]
        if node.get("password"):
...
def outbound_to_v2ray_link(node: dict, name: str) -> str:
    ...
    # HTTP/SOCKS 没有统一的 v2rayN 节点 URI，保持空链接并由 build_group 过滤。
    if t in ("http", "socks"):
        return ""
    if t == "vmess":
...
def classify_and_export(test_results: list):
    ...
    for n in unique_nodes:
        if n.get("outbound"):
            n["outbound"] = dict(n["outbound"])
            n["outbound"].pop("detour", None)
            continue
...
def export_all(unique_nodes, residential, non_residential):
    ...
    def build_group(nodes_list, force_res=False):
        ...
        for idx, item in enumerate(nodes_list, start=1):
            ...
            if not ob:
                continue
            v2ray_link = outbound_to_v2ray_link(ob, name)
            if v2ray_link:
...
def main():
    ...
    # 1. 抓取
    raw_nodes, proxy_candidates = fetch_raw_nodes(include_proxy=True)

    # 2. 解析旧协议 URI；代理源已在独立分支完成统一 outbound
    ...
    def cred_fingerprint(outbound: dict, proto: str) -> str:
        try:
            ...
            if proto == "anytls":
                return f"{outbound.get('password','')}"
            if proto in ("http", "socks4", "socks5"):
                return (f"{proto}|{outbound.get('username', '')}|"
                        f"{outbound.get('password', '')}|{outbound.get('version', '')}")
    ...
    if DEDUP_MAP:
        ...
        for r in test_results:
            key = ((r["server"] or "").lower(), r["port"], r["proto"],
                   cred_fingerprint(r.get("outbound") or {}, r["proto"]))
            result_by_key[key] = r
